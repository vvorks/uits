...
class KeyInfo {
	...
	public constructor(text:string, caps:string|null, r:number, c:number, rs:number, cs:number, action:(me:UiKeyboard, ki:KeyInfo)=>void) {
		this._text = text;
		this._caps = caps;
		this._row = r;
		this._col = c;
...
class UiEditArea extends UiNode {
	...
	public moveCursor(dir:number):void {
		if (this._editStart < this._editEnd) {
			let delta = Math.min(Math.max(this._editStart, this._cursorPos + dir), this._editEnd) - this._cursorPos;
			if (delta != 0) {
				this._cursorPos += delta;
				this.onContentChanged();
			}
		} else {
			let delta = Math.min(Math.max(0, this._cursorPos + dir), this._textContent.length) - this._cursorPos;
			if (delta != 0) {
				this._cursorPos += delta;
