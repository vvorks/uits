...
export class GridPage extends UiPageNode {
	...
	protected initialize():void {
		...
		//行ヘッダ
		b.enter(new UiNode(app, "rowHeader")).style(GROUP_STYLE).th(0, 3).lr(10, 1).hscroll("h");
		...
		b.leave();
		//列ヘッダ
		b.enter(new UiNode(app, "colHeader")).style(GROUP_STYLE).tb(3, 1).lw(0, 10).vscroll("v");
		...
		b.leave();
		//グリッド
		b.enter(new UiNode(app, "grid")).style(GROUP_STYLE).tb(3, 1).lr(10, 1).hscroll("h").vscroll("v");
		for (let row = 0; row < ROW; row++) {
			for (let col = 0; col < COL; col++) {
